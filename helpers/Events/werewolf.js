  ...
  ev.on(
    ...
    async ({ args }) => {
      ...
      if (value === 'create') {
        ...
        await cht.reply(
          'Room berhasil dibuat, ketik *.ww join* untuk bergabung'
        );
        ...
        if (
          ...
          if (!Object.keys(count).length) {
            let randomMayorIdx = Math.floor(
              Math.random() * ww[chat].player.length
            );
            ww[chat].player[randomMayorIdx].isMayor = true;
            ww[chat].mayorStatus = 'done';
            ww[chat].mayor = ww[chat].player[randomMayorIdx].id;
            await Exp.sendMessage(chat, {
              text: `Belum ada vote mayor masuk, jadi mayor dipilih secara acak: @${ww[
                chat
              ].player[randomMayorIdx].id.replace(
                '@s.whatsapp.net',
                ''
              )}.\nPermainan akan segera dimulai.`,
            ...
            let mayorIdx = Number(
              Object.keys(count).find((i) => count[i] === maxVote)
            );
            ww[chat].player[mayorIdx].isMayor = true;
            ww[chat].mayorStatus = 'done';
            ww[chat].mayor = ww[chat].player[mayorIdx].id;
            await Exp.sendMessage(chat, {
              text: `Mayor terpilih adalah @${ww[chat].player[
                mayorIdx
              ].id.replace(
                '@s.whatsapp.net',
                ''
        ...
        await cht.reply(
          'System sedang mengacak rols untuk masing-masing pemain...'
        );
        await sleep(10000);
        roleGenerator(chat, ww);
        await cht.reply(
          'Sistem sudah mengacak role, silahkan cek private chat kamu'
        );
        addTimer(chat, ww);
        startGame(chat, ww);
        for (let i = 0; i < ww[chat].player.length; i++) {
...
Ketik: *.wwpc hunt nomor* untuk memilih target curiga.\n*List Pemain:*:\n${list2}`;
              ...
              let sect = ww[chat].player.map(
                (p) => `Curigai Player ${p.number}`
              );
              await Exp.sendMessage(ww[chat].player[i].id, {
                text,
                mentions: text.extractMentions(),
...
Kamu adalah warga biasa, tapi jika kamu diterawang oleh Seer, kamu akan terlihat seperti *tim jahat*!`;
        ...
        await cht.reply(
          `Vote Mayor kamu untuk player ${args[0]} telah dicatat.`
        );
      } else if (value === 'vote') {
        if (!ww[chat]) return cht.reply('Belum Ada Sesi Permainan');
