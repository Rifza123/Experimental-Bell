...
export default async function on({
  ...
  ev.on({
    ...
    let buff = await func.getBuffer(`${api.xterm.url}/api/maker/triggered-${type}?url=${url}&key=${api.xterm.key}`)
    let res = await exif[type == 'video'?"writeExifVid":'writeExifImg'](buff, {
      packname: 'My brat sticker',
      author: 'Ⓒ' + cht.pushName
    })
    Exp.sendMessage(id, {
