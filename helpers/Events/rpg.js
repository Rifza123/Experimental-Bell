...
async function on({ cht, ev, Exp }) {
	...
	let { berburu, mancing, miningDrops, hargaAwal } = func.inventory.initCfg().rpg
	...
	ev.on({
		...
		const inf = 9999999
		const fields = ['coins', 'healt', 'potion', 'kayu', 'iron', 'gold', 'diamond']
		...
		if (inv.buah)
			for (const x in inv.buah) inv.buah[x] = inf
		if (inv.makanan)
			for (const x in inv.makanan) inv.makanan[x] = inf
		if (inv.tanaman)
			for (const x in inv.tanaman) inv.tanaman[x] = inf
		...
		return cht.reply("♾️ Infinity mode aktif (9999999)! Semua resource utama kamu ditingkatkan.")
	})
	...
	ev.on({
		...
		if (!hargaAwal || typeof HargaAwal !== 'object') return cht.reply(`Item "${item}" tidak tersedia.`)

		const harga = isBuy ? Data.ShopRPG.buy[item] : Data.ShopRPG.sell[item]
		if (!harga) return cht.reply(`Item "${item}" tidak bisa di${isBuy ? 'beli' : 'jual'}.`)
		...
		if (jumlahStr === 'all') {
			jumlah = isBuy ?
				Math.floor(inv.coins / harga) :
				inv[item]
		} else {
			jumlah = Math.max(1, parseInt(jumlahStr) || 1)
		}

		if (jumlah < 1) return cht.reply(`Jumlah ${isBuy ? 'pembelian' : 'penjualan'} tidak valid.`)
		...
		if (isBuy) {
			...
			for (let i = 0; i < jumlah; i++) {
				const pakaiDiskon = sisaDiskon && (sisaDiskon > 0 || sisaDiskon === null) && (expired === 0 || expired > Date.now())
				const hargaSatuan = pakaiDiskon ? Math.max(1, harga - potongan) : harga
				total += hargaSatuan
				if (pakaiDiskon && sisaDiskon !== null) sisaDiskon--
			}

			if (inv.coins < total) return cht.reply(`Koinmu tidak cukup untuk membeli ${jumlah} ${item}.\nSaldo: ${inv.coins}, Harga total: ${total}`)
			...
			if (Data.ShopRPG.diskon[item]) {
				if (Data.ShopRPG.diskon[item].jumlah !== null) {
					...
					if (Data.ShopRPG.diskon[item].jumlah <= 0) delete Data.ShopRPG.diskon[item]
				}
				if (Data.ShopRPG.diskon[item].expired && Data.ShopRPG.diskon[item].expired <= Date.now()) {
...
Terima kasih telah berbelanja!`
		...
		// === JUAL ===
		if (inv[item] < jumlah) return cht.reply(`Kamu hanya punya ${inv[item]} ${item}, tidak cukup untuk dijual.`)
		...
		const updatedText = fileText
			.replace(/diskon: \{[\s\S]*?\},\n  inflasi:/, () => {
				const diskonText = JSON.stringify(Data.ShopRPG.diskon, null, 2).replace(/"([^"]+)":/g, '$1:')
				return `diskon: ${diskonText},\n  inflasi:`
			})
			.replace(/statistik: \{[\s\S]*?\}\n\}/, () => {
				const statistikText = JSON.stringify(Data.ShopRPG.statistik, null, 2).replace(/"([^"]+)":/g, '$1:')
...
- Energy: 1 flow → +100 maxCharge & +200 premium.maxCharge`
		...
		if (!target) return cht.reply("Contoh: *.upgrade pickaxe* atau *.upgrade energy*")

		if (target === 'energy') {
			const flow = inventory.flow || 0
			if (flow < 1) return cht.reply(`Flow kamu tidak cukup.\nDibutuhkan: 1 flow\nFlow kamu: ${flow}`)
			...
			await func.archiveMemories.setItem(sender, 'premium.maxCharge', newPrem)

			return Exp.sendMessage(id, {
				text: `✅ Upgrade Energy Berhasil!\n\n` +
		...
		if (!allowedItems.includes(target)) return cht.reply("Item tidak bisa di-upgrade. Gunakan: pickaxe, sword, ax, fishing_hook, armor")
		...
		if (coins < cost) {
			return cht.reply(`Coins kamu tidak cukup.\nBiaya: ${cost} coins\nCoins kamu: ${coins}`)
		}
		...
		if (bonusCandidates.length > 0) {
			const chosen = bonusCandidates[Math.floor(Math.random() * bonusCandidates.length)]
			if (chosen === 'cooldown') {
				...
				const bonus = target === 'armor' ? Math.floor(func.getRandomValue(10, 51)) : 5
...
- fishing_hook`
		...
		if (!allowedItems.includes(itemToRepair)) {
			return cht.reply("Item tidak bisa diperbaiki. Gunakan: pickaxe / ax / sword / fishing_hook")
		}
	...
	ev.on({
		...
		const input = (cht.q || '').replace(/[|.,]+/g, ' ').split(' ').filter(x => x)
		if (input.length < 2) {
			return cht.reply(`Format: .diskon <item> <potongan> [jumlah]\nContoh:\n• .diskon potion 1000\n• .diskon potion 1000 10`)
		}
		...
		if (!hargaObj || typeof hargaObj.beli !== 'number') {
			return cht.reply(`Item "${item}" tidak memiliki harga beli, tidak bisa diberi diskon.`)
		}

		const baseHarga = Data.ShopRPG.buy[item] || hargaObj.beli
		if (isNaN(potongan) || potongan < 1) return cht.reply(`Potongan harus berupa angka positif.`)
		if (potongan >= baseHarga) return cht.reply(`Potongan tidak boleh lebih besar atau sama dengan harga (${baseHarga}).`)

		for (const i in Data.ShopRPG.diskon) {
			const d = Data.ShopRPG.diskon[i]
			const expired = d.expired || 0
			const sisa = d.jumlah
			const sudahHabis = (expired > 0 && expired <= Date.now()) || (sisa !== null && sisa <= 0)
	...
	ev.on({
		...
		if (func.getRandomValue(0, 1) < 0.4) {
			const bonusRate = isPremium ? func.getRandomValue(0, 0.25) : func.getRandomValue(0, 0.15)
			bonusEnergy = Math.floor(baseEnergy * bonusRate)
			totalEnergy += bonusEnergy
		}
		await memories.setItem(sender, 'energy', (user.energy || 0) + totalEnergy)
		...
		for (let reward of dropPool) {
			if (func.getRandomValue(0, 1) < reward.rate) {
				if (reward.type === 'premium_bonus') {
					const minutes = Math.floor(func.getRandomValue(reward.min, reward.max + 1))
					...
					let amount = Math.floor(func.getRandomValue(reward.min, reward.max + 1))
		...
		teks += rewardGiven ? `+ Hadiah: ${rewardGiven}` : `(Tidak ada drop kali ini)`
...
Daftar: pickaxe, ax, sword, fishing_hook, armor`
		...
		const text = `*Berhasil membuat:* ${itemToCraft}\n` +
			`- Iron: -${bahan.iron}\n- Kayu: -${bahan.kayu}\n\n` +
			`*Detail item:*\n${detail}`
	...
	ev.on({
		...
		if (inv.healt <= 0) return cht.reply(`❤️ Kamu sedang sekarat!\nGunakan *.heal* terlebih dahulu.`)
		if (!pickaxe) return cht.reply("Kamu belum punya pickaxe. Gunakan `.craft pickaxe` dulu.")
		if (pickaxe.durability <= 0) return cht.reply("Pickaxe kamu rusak. Gunakan *.repair pickaxe* terlebih dahulu.")
		...
		if (remaining > 0) {
			...
			return cht.reply(`⏳ Tunggu *${dur.minutes}m ${dur.seconds}s* lagi untuk menambang.`)
		}
		...
		// === Luka Tambang
		const lukaMenambang = [
			"Terjatuh di dalam tambang gelap", "Luka oleh alat pertambangan", "Batuan jatuh menimpa",
			"Tersengat listrik saat menambang", "Terkunci di dalam tambang", "Kehabisan oksigen di dalam tambang",
			"Luka oleh pecahan kaca", "Ketiban bebatuan saat menambang", "Mengalami runtuhan tambang",
			"Tergelincir di lorong tambang", "Terjebak di bawah tumpukan batuan", "Terperangkap dalam lubang sempit",
			"Ketiban alat berat saat menambang", "Terjatuh dari ketinggian di dalam tambang", "Kehabisan makanan dan air di dalam tambang",
			"Mengalami keracunan gas tambang", "Terperosok ke dalam lubang tambang", "Luka oleh ledakan di dalam tambang",
			"Tersandung kabel listrik di dalam tambang", "Luka oleh bahan kimia tambang", "Terkurung di dalam tambang terbengkalai",
			"Terjebak dalam terowongan tambang yang runtuh", "Tersengat aliran air di dalam tambang", "Luka oleh alat bor tambang",
			"Terkunci di dalam ruang bawah tanah", "Kecelakaan kereta bawah tanah", "Tergantung di dalam terowongan tambang",
			"Terjatuh dari jembatan di dalam tambang", "Terkunci di dalam kabin lift tambang", "Luka oleh mesin penggali di dalam tambang"
		]
		const kejadianLuka = lukaMenambang[Math.floor(Math.random() * lukaMenambang.length)]
		...
		const durArmor = armor ? persentase(armor.durability, armor.maxDurability) : 0
	...
	ev.on({
		...
		if (inv.healt <= 0) return cht.reply(`❤️ Kamu sedang sekarat!\nGunakan *.heal* terlebih dahulu.`)
		if (!sword) return cht.reply("Kamu belum punya sword. Gunakan `.craft sword` dulu.")
		if (sword.durability <= 0) return cht.reply("Sword kamu rusak. Gunakan *.repair sword* terlebih dahulu.")
		...
		if (remaining > 0) {
			const dur = func.formatDuration(remaining)
			return cht.reply(`⏳ Tunggu *${dur.minutes}m ${dur.seconds}s* lagi untuk berburu.`)
		}
		...
		let total = Math.floor(Math.random() * (maxJumlah - minJumlah + 1)) + minJumlah
		...
		// Luka
		const lukaList = [
			"Terpeleset akar pohon", "Diserang binatang buas", "Jatuh dari tebing", "Tersesat di hutan", "Tertusuk duri tajam"
		]
		let luka = false,
			kejadian = "",
			dmg = 0,
	...
	ev.on({
		...
		if (!pancing) return cht.reply("Kamu belum punya pancing. Gunakan `.craft fishing_hook` dulu.")
		if (pancing.durability <= 0) return cht.reply("Pancing kamu rusak. Gunakan `.repair fishing_hook* terlebih dahulu.")
		...
		if (remaining > 0) {
			const dur = func.formatDuration(remaining)
			return cht.reply(`⏳ Tunggu *${dur.minutes}m ${dur.seconds}s* lagi untuk mancing.`)
		}
	...
	ev.on({
		...
		if (potions <= 0) {
			return cht.reply(`⚠️ Kamu tidak punya potion untuk menyembuhkan diri.\nGunakan .buy potion atau cari di misi.`)
		}

		if (healt >= 100) {
			return cht.reply(`❤️ Healt kamu sudah penuh (${healt}/100). Tidak perlu menyembuhkan.`)
		}
		...
		const text = `❤️ Kamu telah menggunakan 1 potion.\n` +
