...
export default async function on({
    ...
    opts[k[0]].includes(a) ?
      ...
      (async (v) => {
        ...
        if (!v) {
          ...
          let {
            key
          } = await cht.reply(`*Silahkan reply pesan ini dengan ${isDesc ? 'deskripsi':'nama' } group yang baru*\n> _Expired on ${date} (${area})_`)
      ...
      (async () => {
        ...
        if (!(is.image || is.quoted.image)) {
          ...
          let {
            key
          } = await cht.reply(`*Silahkan reply pesan ini dengan foto yang akan dijadikan thumbnail/foto profile group!*\n> _Expired on ${date} (${area})_`)
          let qcmds = memories.getItem(sender, "quotedQuestionCmd") || {}
          qcmds[key.id] = {
            emit: `${cht.cmd} setpp`,
            exp,
  ...
  ev.on({
    ...
    let {
      status
    } = (await Exp.groupParticipantsUpdate(id, cht.mention, cht.cmd == "kick" ? "remove" : "add"))[0]
  ...
  ev.on({
    ...
    let actions = ["welcome", "antilink", "antitagall", "mute", "antibot", "playgame", "jadwalsholat", "onlyadmin", "antidelete"]
...
\`Kami sangat berharap panduan ini dibaca dengan teliti agar tidak lagi menanyakan kepada admin terkait cara penggunaanya, terimakasih\`
 `
  ...
  function calculateSoon(timeZone, hh, mm) {
    ...
    let nowDate = new Date(new Date().toLocaleString("en-US", {
      timeZone
    }));
    ...
    if (type == 'add') {
      ...
      for (let i of chatDb.schedules) {
        ...
        let {
          remainingHours,
          remainingMinutes
        } = calculateSoon(i.timeZone, hh, mm)
        txt += `
# \`${i.time}, ${i.timeZone}\`
- ⚡ Action: ${i.action}
- 💬 Pesan: ${i.msg}
...
`
  ...
  ev.on({
    ...
    try {
      ...
      cht.reply(`Success ${cht.cmd} user @${cht.mention[0].replace(from.sender, '')}`, {
