...
export default async function on({ Exp, ev, cht, is }) {
  ev.on(
    { cmd: ['wwpc'], tag: 'game', listmenu: ['wwpc'] },
    async ({ args }) => {
      ...
      for (let players of allPlayers)
        for (let p of players) playerSessionMap[p.id] = p.sesi;
      ...
      if (!playerOnGame(sender, ww))
        return cht.reply('Kamu tidak dalam sesi game');
      if (dataPlayer(sender, ww).isdead === true)
        return cht.reply('Kamu sudah mati');
      if (!target || isNaN(target))
        return cht.reply('Masukan nomor player. Contoh: .wwpc kill 1');

      let byId = getPlayerById2(sender, parseInt(target), ww);
      if (!byId) return cht.reply('Player tidak terdaftar');
      if (byId.db.isdead === true) return cht.reply('Player sudah mati');
      if (byId.db.id === sender && value !== 'deffense')
        return cht.reply('Tidak bisa untuk diri sendiri');

      if (value === 'kill') {
        const player = dataPlayer(sender, ww);
        if (
          player.role !== 'werewolf' &&
          !(player.role === 'traitor' && player.isTraitorActive)
        )
          return cht.reply('Peran ini bukan untuk kamu');
        if (byId.db.role === 'sorcerer')
          return cht.reply('Tidak bisa membunuh teman');
        ...
        for (let i of sames)
          await Exp.sendMessage(i, {
            text: `Werewolf memilih target ${parseInt(target)}`,
            mentions: [sender],
          });
        return;
      }
      ...
      if (value === 'dreamy' || value === 'seer') {
        ...
        if (dataPlayer(sender, ww).status)
          return cht.reply('Skill sudah dipakai malam ini');
      ...
      if (value === 'hunt') {
        if (dataPlayer(sender, ww).role !== 'hunter')
          return cht.reply('Peran ini bukan untuk kamu');
      ...
      if (value === 'elixir') {
        if (dataPlayer(sender, ww).role !== 'witch')
          return cht.reply('Bukan peranmu');
        if (!room || room.day == 1)
          return cht.reply('Tidak bisa digunakan malam pertama');
        if (!dataPlayer(sender, ww).witch.elixir)
          return cht.reply('Elixir habis');
        if (dataPlayer(sender, ww).status)
          return cht.reply('Sudah pakai potion malam ini');
      ...
      if (value === 'poison') {
        ...
        if (!room || room.day == 1)
          return cht.reply('Tidak bisa digunakan malam pertama');
        if (!dataPlayer(sender, ww).witch.poison)
          return cht.reply('Poison habis');
        if (dataPlayer(sender, ww).status)
          return cht.reply('Sudah pakai potion malam ini');
        if (room.dead.includes(parseInt(target)))
          return cht.reply('Player sudah mati');
