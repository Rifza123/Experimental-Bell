...
export default async function on({ cht, Exp, store, ev, is }) {
    ...
    ev.on({ 
        ...
        Exp.sendMessage(cht.id, { image: { url: api.rifza.url + "/api/pinterest-v2?query="+cht.q } }, { quoted: cht })
	})
	...
	ev.on({ 
        ...
        Exp.sendMessage(cht.id, { video: { url: _pin }, caption: pin.title, mimetype: "video/mp4"}, { quoted: cht })                
        } catch (e){
