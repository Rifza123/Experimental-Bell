...
export async function stopJadibot({
  ...
  const ownersList = (Data.jadibot[slot]?.owners || []).map((o) =>
...
export async function checkExpiredJadibots() {
  ...
  for (const slot of Object.keys(Data.jadibot)) {
    ...
    if (info && info.expired && info.expired > 0 && now > info.expired) {
      console.log(
        `[JADIBOT EXPIRED] Deleting expired sub-bot slot #${slot} (${info.botNumber})`
      );
...
async function jadibot({
  ...
  if (Exp?.isJadibot) {
    console.log(
      `[JADIBOT] Blocked nested jadibot request on sub-bot (${Exp.jadibotNumber || 'sub-bot'})`
    );
    try {
      if (cht && cht.reply) {
  ...
  try {
    ...
    Data.jadibot[slot].reconnectCount =
      (Data.jadibot[slot].reconnectCount || 0) + 1;
    ...
    _Exp.ev.on('connection.update', async (update) => {
      ...
      if (
        qr &&
        !_Exp.authState.creds.registered &&
        cht &&
        cht.id &&
        !_Exp._qrSent
      ) {
        ...
        try {
          ...
          const jdbLang = Data.infos.jadibot;
          try {
            const imageMessage = await Exp.func.uploadToServer(
              qrBuffer,
              'image'
            );
            ...
            console.error(
              '[JADIBOT] Relay interactive QR failed, sending plain image:',
              btnErr
            );
            await Exp.sendMessage(
              cht.id,
              { image: qrBuffer, caption: jdbLang.qrCode(realNumber, slot) },
              { quoted: cht }
            );
          }
        } catch (e) {
      ...
      if (connection === 'close') {
        ...
        if (
          !_Exp.authState.creds.registered &&
          (Data.jadibot?.[slot]?.reconnectCount || 0) > 3
        ) {
          if (Data.jadibot?.[slot]) Data.jadibot[slot].status = 'offline';
          console.log(
            `[JADIBOT] Unregistered reconnect limit reached for ${realNumber} (Slot ${slot})`
          );

          if (cht && cht.id && !_Exp._disconnectedNotified) {
            _Exp._disconnectedNotified = true;
            const timeoutMsg =
              '⛔ *PENAUTAN GAGAL / TIMEOUT*\n\n' +
              `• *Nomor:* ${realNumber}\n` +
              `• *Slot:* ${slot}\n` +
      ...
      if (update.connection === 'open') {
        ...
        if (!_Exp._energyDeducted && userSender) {
          ...
          try {
            const res = defaultFunc.archiveMemories.reduceEnergy(
              userSender,
              1500
            );
      ...
      for (let message of messages) {
        ...
        if (
          ...
          if (message?.message) {
            ...
            console.log('[META_RELAY_PAYLOAD_JSON (Jadibot)]\n' + relayJson);
          } else {
            ...
            console.log('[META AI MSG JSON (Jadibot)]\n' + cleanStr);
          }
          if (isMeta) continue;
        }
...
export function resetJadibotDb(ownerNum) {
  ...
  Data.preferencesBot ??= {};
  Data.preferencesBot[info.botNumber] =
    Data.jadibotDb[info.botNumber].preferences;
  return true;
}
...
export function resolveJadibotSlot(target, senderNum) {
  ...
  if (target) {
    ...
    const foundByNumber = Object.keys(Data.jadibot).find((k) => {
      const bNum = String(Data.jadibot[k]?.botNumber || '').replace(
        /[^0-9]/g,
        ''
      );
      ...
      return (
        bNum === raw ||
        oNum === raw ||
        (bNum && raw && bNum.endsWith(raw)) ||
        (bNum && raw && raw.endsWith(bNum))
      );
    });
  ...
  if (senderNum) {
    ...
    const foundBySender = Object.keys(Data.jadibot).find((k) => {
      const oNum = String(Data.jadibot[k]?.owner || '').replace(/[^0-9]/g, '');
      const owners = (Data.jadibot[k]?.owners || []).map((o) =>
        String(o).replace(/[^0-9]/g, '')
      );
