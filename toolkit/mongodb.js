...
class MongoCollection {
    ...
    async _ensureConnection() {
        if (!this._db?.client?.topology?.isConnected?.()) {
            console.warn(`[MongoCollection] Lost connection, attempting to reconnect...`)
    ...
    async get(id) {
      ...
      try {
        ...
        console.error(`Error fetching data (id: ${id}) in collection ${this.collection}:`, err);
        return undefined
      }
    }
    ...
    async set(id, data) {
      ...
      try {
        ...
        console.error(`Error saving data (id: ${id}) to collection ${this.collection}:`, err);
        return undefined
      }
    }
