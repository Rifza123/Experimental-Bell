...
async function detector({ Exp, store }) {
    ...
    setupWatcher(path.resolve(fol[7], '**/*.js'), 2000, async () => {
        try {
    ...
    async function keyChecker(url, key) {
        try {
            ...
            if (Data.notify.h == 1 && Exp.authState) {
                ...
                if (!res.ok) return Exp.sendMessage(own, {
...
Semoga puasa kita diterima Allah dan diberikan kekuatan serta kelancaran sepanjang hari. Aamiin.` :
                    ...
                    data.now !== 'imsak' && await Exp.sendMessage(id, {
                        audio: {
                            url: data.adzan
                        },
                        mimetype: "audio/mpeg",
        ...
        */
        ...
        for (const item in Data.ShopRPG.hargaAwal) {
            ...
            if (!inflasiAktif) {
                ...
                if (typeof harga.jual === 'number') Data.ShopRPG.sell[item] = harga.jual
                continue
            }
            ...
            if (typeof harga.beli === 'number') {
                ...
                const faktorBeli = Math.max(0.5, Math.min(faktor * permintaan, faktor * 1.5))
